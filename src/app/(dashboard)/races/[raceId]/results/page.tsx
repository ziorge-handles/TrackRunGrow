...
export default function ResultEntryPage() {
  ...
  useEffect(() => {
    ...
    // Fetch track events
    fetch('/api/races')
      .then(() => {}) // placeholder; we'll use a simple list
      .catch(console.error)
  }, [raceId])

  const updateRow = (index: number, field: keyof ResultRow, value: string | boolean) => {
  ...
  const handleSave = async () => {
    ...
    const validRows = rows.filter((r) => r.athleteId && (r.resultValue || r.dnf || r.dns))

    if (validRows.length === 0) {
      setError('No valid results to save. Make sure athletes are selected and results are entered.')
      setSaving(false)
      return
    }
    ...
    try {
      const res = await fetch(`/api/races/${raceId}/results`, {
        ...
        body: JSON.stringify({
          results: validRows.map((r) => ({
            athleteId: r.athleteId,
            resultValue: parseFloat(r.resultValue) || 0,
      ...
      const data = await res.json() as { saved?: number; newPRs?: string[]; error?: string }
      ...
      <div className="max-w-2xl mx-auto py-16 text-center">
        ...
        {newPRs.length > 0 && (
          <div className="mt-4 bg-amber-50 border border-amber-200 rounded-xl p-4 text-left">
            ...
            {newPRs.map((pr) => (
              <p key={pr} className="text-sm text-amber-700">• {pr}</p>
            ))}
          </div>
        )}
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        ...
        {race && <p className="text-sm text-gray-500 mt-1">{race.name}</p>}
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <p className="text-xs text-gray-500">
            Paste tab-separated values with columns: Athlete Name | Place | Result
          </p>
          <textarea
            value={pasteText}
          ...
          />
          <Button variant="outline" size="sm" onClick={parsePaste} disabled={!pasteText.trim()}>
            Parse Rows
          </Button>
        </CardContent>
      </Card>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  <th className="text-left px-4 py-2 text-xs font-medium text-gray-500">Athlete</th>
                  {race?.sport === 'TRACK' && (
                  ...
                  <th className="text-left px-4 py-2 text-xs font-medium text-gray-500">Place</th>
                  <th className="text-left px-4 py-2 text-xs font-medium text-gray-500">DNF/DNS</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {rows.map((row, i) => (
                  <tr key={i} className={row.dnf || row.dns ? 'opacity-60' : ''}>
                    <td className="px-4 py-2">
                      <select
                        value={row.athleteId}
                        onChange={(e) => updateRow(i, 'athleteId', e.target.value)}
                        className="w-full rounded border border-gray-200 px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500"
                      >
                        <option value="">{row.athleteName || 'Select athlete...'}</option>
                        {athletes.map((a) => (
                    ...
                    {race?.sport === 'TRACK' && (
                      <td className="px-4 py-2">
                        <select
                          ...
                          onChange={(e) => updateRow(i, 'trackEventId', e.target.value)}
                          className="w-full rounded border border-gray-200 px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500"
                        >
                    ...
                    <td className="px-4 py-2">
                      <Input
                        ...
                        onChange={(e) => updateRow(i, 'resultValue', e.target.value)}
                        placeholder={race?.sport === 'XC' ? 'e.g. 952.5' : 'seconds or meters'}
                        className="h-8 text-sm font-mono"
                        disabled={row.dnf || row.dns}
                      />
                    </td>
                    <td className="px-4 py-2">
                      <Input
                      ...
                      <div className="flex gap-2">
                        <label className="flex items-center gap-1 text-xs">
                          <input
                            type="checkbox"
                            checked={row.dnf}
                            onChange={(e) => updateRow(i, 'dnf', e.target.checked)}
                          />
                          DNF
                        </label>
                        <label className="flex items-center gap-1 text-xs">
                          <input
                            type="checkbox"
                            checked={row.dns}
                            onChange={(e) => updateRow(i, 'dns', e.target.checked)}
                          />
                          DNS
                        </label>
                      </div>
